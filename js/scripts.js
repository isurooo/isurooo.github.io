...
window.initChat = async (collection, addDoc, onSnapshot, query, orderBy, serverTimestamp) => {
    ...
    const displayMessage = (sender, text) => {
        ...
        messageDiv.classList.add('chat-message', sender, 'p-3', 'rounded-lg', 'max-w-[80%]', 'shadow-sm');
    ...
    onSnapshot(query(messagesCollectionRef, orderBy("timestamp")), (snapshot) => {
        snapshot.docChanges().forEach((change) => {
            if (change.type === "added") {
                ...
                if (!chatHistory.some(msg => msg.text === message.text && msg.role === message.sender)) {
    ...
    const sendMessageToGemini = async (userMessage) => {
        ...
        chatLoadingIndicator.innerHTML = '<div class="ai-loading-spinner"></div> AI is typing...';
        ...
        const payload = {
            contents: [{ role: "user", parts: [{ text: aiInstructions }] }, ...chatHistory]
        };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (chatButton) {
        chatButton.addEventListener('click', () => {
            ...
            if (chatModal) {
                ...
                console.log("chatModal hidden state:", chatModal.classList.contains('hidden')); // Debugging log
            } else {
    ...
    // Simple resume download button action (can be replaced with actual download logic)
    document.getElementById('download-resume-btn').addEventListener('click', (e) => {
        e.preventDefault();
        showTemporaryMessageBox('Resume download functionality coming soon!', 'Download Info');
    });
});